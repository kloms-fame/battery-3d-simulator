...
export class Engine3D {
    ...
    public loadBatteryPack(doc: any): void {
        ...
        const radius = 9;
        const height = 65;
        const cellGeo = new THREE.CylinderGeometry(radius, radius, height, 32);
        const capGeo = new THREE.CylinderGeometry(radius - 1, radius - 1, 2, 32);
        ...
        cells.forEach((c: any) => {
            ...
            const topCap = new THREE.Mesh(capGeo, c.polarity === 'positive' ? posMat : negMat);
            ...
            const bottomCap = new THREE.Mesh(capGeo, c.polarity === 'positive' ? negMat : posMat);
